<template>
  <div class="registry-vue">
    <van-nav-bar title="用户登录" left-text="返回" right-text="注册" left-arrow @click-left="onBack" @click-right="onRegistry" />
    <van-cell-group class="reg-from">
      <van-field v-model="userName" required clearable label="用户名" icon="error" placeholder="请输入用户名" @click-icon="userName=''" :error-message="userNameErrorMsg" />
